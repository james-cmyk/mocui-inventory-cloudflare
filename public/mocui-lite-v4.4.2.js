...
(function(){
  ...
  const base={
    ...
    renderLoanFormModal,openProductForm,openCustomerSelector,syncSaleFormToDraft,saveSale,saleCard,cancelSale,openSaleDetail,openModal,renderReports
  };
  ...
  async function renderDashboardLite(){
    ...
    const todayProfit=todaySales.reduce((a,s)=>a+(typeof saleGrossProfit==='function'?num(saleGrossProfit(s)):0),0);
    $('#main').innerHTML=`<div class="v44-home">${searchBarHTML('v44HomeSearch','搜货号、圈口、名称、同行、历史记录')}
      <div class="v44-metrics v441-compact-metrics">
        <button class="v44-metric primary-number" id="v44TodaySales"><span>今日成交</span><strong>${money(todayAmount)}</strong><small>${todaySales.length} 笔</small></button>
        <button class="v44-metric primary-number" id="v441TodayProfit"><span>今日利润</span><strong>${money(todayProfit)}</strong><small>按已记录成本</small></button>
      ...
      ${(overdue.length||dueXhs.length)?`<section class="v44-attention"><div class="v44-section-title"><strong>需要留意</strong><span>只显示需要处理的事项</span></div>${dueXhs.length?`<button id="v44DueXhs" type="button"><span>小红书订单到确认日</span><b>${dueXhs.length} 单 ›</b></button>`:''}${overdue.length?`<button id="v44OverdueLoans" type="button"><span>调借已超期</span><b>${overdue.length} 单 ›</b></button>`:''}</section>`:''}
      <section><div class="v44-section-title"><strong>常用操作</strong><span>高频入口</span></div><div class="v44-actions v441-compact-actions">
        <button id="v44Sale" class="primary"><span>销售开单</span><small>普通 / 小红书</small></button>
        <button id="v44Product"><span>新增商品</span><small>正式商品档案</small></button>
        <button id="v44Loan"><span>新增调借</span><small>借出 / 调入</small></button>
        <button id="v44Pass"><span>过手单</span><small>不动库存</small></button>
        <button id="v44Content"><span>内容工作台</span><small>素材与文案</small></button>
        <button id="v44Stocktake"><span>库存盘点</span><small>核对库存</small></button>
      </div></section>
    ...
    $q('#v44TodaySales').onclick=()=>navigate('sales');
  ...
  renderReports=async function(){
    const r=await base.renderReports.apply(this,arguments);
    const main=$q('#main');
    if(main&&!$q('#v442ReportTip'))main.insertAdjacentHTML('afterbegin','<div id="v442ReportTip" class="v442-report-tip">手机上排名表可左右滑动查看更多</div>');
    scheduleNavFix();
    return r;
  };
  ...
  openModal=function(){const r=base.openModal.apply(this,arguments);requestAnimationFrame(normalizeModalChrome);return r;};
  ...
  // ---------- 启动 ----------
  ensurePrimaryNav();
  setTimeout(ensurePrimaryNav,120);setTimeout(ensurePrimaryNav,650);
  window.addEventListener('pageshow',ensurePrimaryNav,{passive:true});
  window.MocuiLite442={version:VERSION,openGlobalSearch,startVoice,isXhsPending,autoDetectXhsCustomer};
})();
