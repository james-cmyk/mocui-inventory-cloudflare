'use strict';
(()=>{
  const VERSION='4.3.0', MOBILE='(max-width:759px)';
  let raf=0;
  ...
  function lock(){
    ...
    /* Hard lock: 60px CSS dock, with no extra bottom safe-area added. */
    imp(nav,'position','fixed'); imp(nav,'left','0'); imp(nav,'right','0');
    imp(nav,'bottom','0'); imp(nav,'top','auto'); imp(nav,'inset','auto 0 0 0');
    imp(nav,'width','100%'); imp(nav,'height','60px'); imp(nav,'min-height','60px');
    imp(nav,'max-height','60px'); imp(nav,'flex','0 0 60px');
    imp(nav,'margin','0'); imp(nav,'padding','2px 8px 2px'); imp(nav,'transform','none');
  ...
  function verify(){
    ...
    const r=nav.getBoundingClientRect(), expected=60;
