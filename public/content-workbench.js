...
(() => {
  ...
  async function renderContentHub(){
    ...
    const draw=()=>{
      ...
      $$('.workbench-product-card [data-action]').forEach(btn=>btn.onclick=async e=>{e.stopPropagation();const p=byId.get(btn.dataset.id);if(!p)return;const action=btn.dataset.action;if(action==='edit')navigate('product-content',{id:p.id});else if(action==='download'){const list=mediaOf(p),hub=contentOf(p),momentsText=String(hub.copies?.moments||'').trim();if(!(await saveViaShortcut(list,p,{btn,title:p.name,copyText:momentsText}).catch(()=>false)))await prepareInlineFiles(list,p,{btn,title:p.name});}else if(action==='store11'){const list=mediaOf(p).filter(m=>m.type==='image'),hub=contentOf(p),opt=applyXhsTagOptimization(p,hub,hub.copies?.xhsTitle||'',hub.copies?.xhs||''),xhsBody=String(opt.body||'').trim();if(!(await saveViaShortcut(list,p,{store11:true,btn,title:`${p.name} 店铺1:1`,copyText:xhsBody}).catch(()=>false)))await prepareInlineFiles(list,p,{store11:true,btn,title:`${p.name} 店铺1:1`});}else if(action==='copy')await openWorkbenchCopy(p);else if(action==='share')await openWorkbenchShare(p);});
    };
  ...
  async function renderProductContent(){
    ...
    const bindContentPage=()=>{
      ...
      $('#shareAllMedia').onclick=async e=>{syncHubFromFields();await saveProductContent(p,hub);const list=mediaOf(p),momentsText=String(hub.copies.moments||'').trim();if(!list.length&&p.image&&!ownMediaUrl(p.image)){showToast('请先把秦丝外链主图转存到自己的R2');return;}if(!(await saveViaShortcut(list,p,{btn:e.currentTarget,title:p.name,copyText:momentsText}).catch(()=>false)))await prepareInlineFiles(list,p,{btn:e.currentTarget,title:p.name}).catch(()=>{});};
      $('#export11').onclick=async e=>{syncHubFromFields();const opt=applyXhsTagOptimization(p,hub,hub.copies.xhsTitle,hub.copies.xhs);hub.copies.xhsTitle=opt.title;hub.copies.xhs=opt.body;hub.xhsTags={items:opt.rows,updatedAt:new Date().toISOString()};await saveProductContent(p,hub);const list=mediaOf(p).filter(m=>m.type==='image'),xhsBody=String(opt.body||'').trim();if(!(await saveViaShortcut(list,p,{store11:true,btn:e.currentTarget,title:`${p.name} 店铺1:1`,copyText:xhsBody}).catch(()=>false)))await prepareInlineFiles(list,p,{store11:true,btn:e.currentTarget,title:`${p.name} 店铺1:1`}).catch(()=>{});};
...
// v3.6.1 快捷指令剪贴板修复：下载=朋友圈文案；店铺1:1=小红书正文+话题
})();
