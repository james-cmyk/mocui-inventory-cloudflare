...
(function(){
  ...
  async function compactDashboard430(){
    ...
    const badge=document.createElement('div');badge.id='formal430VersionBadge';badge.className='formal430-version-badge';badge.textContent='v4.3.0 · 正式整理版';main.appendChild(badge);
  }
  ...
  async function formalizeMore430(){
    ...
    const foot=document.createElement('div');foot.className='formal430-release-note';foot.innerHTML='<strong>漠翠进销存 v4.3.0</strong><span>4.2 日常业务效率优化已收口 · 底层业务账本不迁移</span>';main.appendChild(foot);
  }
  ...
  // 对首次启动和通过浏览器历史恢复的页面也保持正式版导航。
  const observer=new MutationObserver(()=>{ensureNav430();});
  const start=()=>{ensureNav430();const nav=document.querySelector('.bottom-nav');if(nav)observer.observe(nav,{childList:true});};
