'use strict';
(()=>{
  const VERSION='4.7.0', MOBILE='(max-width:759px)', DOCK=54;
  let raf=0;
  function safeInsets(){
  ...
  function calibrate(){
    ...
    const s=safeInsets();
    /* screen.height is the physical app canvas in CSS px on iOS. innerHeight is
       the WebKit layout viewport. Remove the top safe inset: what remains is
       only the bottom canvas that WebKit did not give to the layout viewport. */
  ...
  function verify(){
    ...
    return {version:VERSION,standalone:standalone(),screenHeight:screen.height,innerHeight,clientHeight:document.documentElement.clientHeight,visualViewportHeight:visualViewport?.height??null,safeTop:s.top,safeBottom:s.bottom,canvasTail:getComputedStyle(document.documentElement).getPropertyValue('--mocui-ios-canvas-tail').trim(),appBottom:ar.bottom,navTop:nr.top,navBottom:nr.bottom,navHeight:nr.height,expectedDockHeight:DOCK};
  }
  function start(){calibrate();window.MocuiShellFramework={version:VERSION,verify,calibrate};window.addEventListener('resize',schedule,{passive:true});window.addEventListener('orientationchange',schedule,{passive:true});window.addEventListener('pageshow',schedule,{passive:true});document.addEventListener('visibilitychange',()=>{if(!document.hidden)schedule()},{passive:true})}
