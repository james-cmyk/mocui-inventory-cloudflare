...
async function dbPut(store,value,silent=false){ const r=await reqP(db.transaction(store,'readwrite').objectStore(store).put(value)); if(!silent&&!window.__cloudImporting)window.CloudSync?.schedule(); return r; }
async function dbAdd(store,value,silent=false){ const r=await reqP(db.transaction(store,'readwrite').objectStore(store).add(value)); if(!silent&&!window.__cloudImporting)window.CloudSync?.schedule(); return r; }
async function dbDelete(store,id,silent=false){ const r=await reqP(db.transaction(store,'readwrite').objectStore(store).delete(id)); if(!silent&&!window.__cloudImporting)window.CloudSync?.schedule(); return r; }
async function dbClear(store,silent=false){ const r=await reqP(db.transaction(store,'readwrite').objectStore(store).clear()); if(!silent&&!window.__cloudImporting)window.CloudSync?.schedule(); return r; }
...
function imageThumb(p){ return p.image?`<img class="thumb" src="${p.image}" alt="">`:`<div class="thumb placeholder">玉</div>`; }
...
async function renderProducts(){
  ...
  const qty=products.reduce((s,p)=>s+n(p.stock),0), cost=products.reduce((s,p)=>s+n(p.stock)*n(p.costPrice),0);
  $('#main').innerHTML=`
    <div class="grid-3">
      <div class="metric compact"><div class="label">商品数量</div><div class="value">${products.length}</div></div>
  ...
  const draw=()=>{
    ...
    const stockOK=p=>!stock||(stock==='in'&&n(p.stock)>0)||(stock==='low'&&n(p.stock)<=1)||(stock==='out'&&n(p.stock)<=0);
...
function saleCard(s){
  const linked=s.sourceType==='loan_sale'||s.items?.some(i=>i.fromLoan);
  return `<div class="card sale-card" data-id="${s.id}"><div style="display:flex;justify-content:space-between;gap:10px"><div><div class="item-title">${esc(s.customerName||'散客')} · ${esc(s.orderNo)}</div><div class="item-meta">${fmtDateTime(s.createdAt)} · ${s.items.length} 种商品 ${linked?'· 借调售出':''}</div></div><div class="item-right"><strong>${fmtMoney(s.finalAmount)}</strong><span class="badge ${s.status==='active'?'success':'danger'}">${s.status==='active'?'有效':'已撤销'}</span></div></div><div class="item-meta" style="margin-top:8px">${s.items.slice(0,3).map(i=>`${esc(i.productName)}×${fmtInt(i.qty)}`).join('、')}${s.items.length>3?'…':''}</div>${linked?`<div class="linked-source-tag">⇄ 来源：${esc(s.sourceLoanNo||s.items.find(i=>i.loanNo)?.loanNo||'借调单')}</div>`:''}<div class="btn-row" style="margin-top:10px">${s.status==='active'?`<button class="btn small danger cancel-sale" data-id="${s.id}">撤销销售</button>`:`<button class="btn small success restore-sale" data-id="${s.id}">恢复销售单</button><button class="btn small secondary duplicate-sale" data-id="${s.id}">复制重新开单</button>`}</div></div>`;
}
...
async function cancelSale(id){
  const s=await dbGet('sales',id);if(!s||s.status!=='active')return;if(!await confirmDialog('确定撤销这张销售单？普通销售会恢复仓库库存；借调售出会恢复为借调未处理数量。'))return;
  try{
...
async function restoreSale(id){
  const s=await dbGet('sales',id);if(!s||s.status!=='cancelled')return;
  try{
    for(const i of s.items){
      if(i.fromLoan&&i.loanId){
...
async function renderLoanFormModal(){
  ...
  const history=(await dbAll('loans')).sort((a,b)=>new Date(b.date||b.createdAt)-new Date(a.date||a.createdAt));
  const peopleMap=new Map();
  history.forEach(row=>{
    const name=String(row.person||'').trim();if(!name)return;
    const old=peopleMap.get(name)||{name,count:0,lastDate:row.date||row.createdAt,open:0};
    old.count+=1;if(loanIsOpen(row))old.open+=1;if(new Date(row.date||row.createdAt)>new Date(old.lastDate))old.lastDate=row.date||row.createdAt;
...
async function renderSettings(){
  setHeader('数据与设置','本地备份、合同抬头与恢复');
  const counts={};for(const s of STORES)counts[s]=(await dbAll(s)).length;const profile=await getLegalProfile();
  $('#main').innerHTML=`<div class="card"><div class="card-title">合同抬头（仅保存在本机）</div><div class="notice">用于自动生成借调协议和调拨交接单。请填写真实签约主体，不要只写店铺昵称。</div><form id="legalProfileForm"><div class="form-group"><label class="form-label">甲方真实姓名/公司名称</label><input id="setPartyAName" class="input" value="${esc(profile.partyAName)}"></div><div class="form-group"><label class="form-label">身份证号/统一社会信用代码</label><input id="setPartyAIdNo" class="input" value="${esc(profile.partyAIdNo)}"></div><div class="form-row"><div class="form-group"><label class="form-label">联系电话</label><input id="setPartyAPhone" class="input" value="${esc(profile.partyAPhone)}"></div><div class="form-group"><label class="form-label">交接地点</label><input id="setDeliveryPlace" class="input" value="${esc(profile.defaultDeliveryPlace)}"></div></div><div class="form-group"><label class="form-label">住所/经营地址</label><input id="setPartyAAddress" class="input" value="${esc(profile.partyAAddress)}"></div><div class="form-group"><label class="form-label">默认争议管辖</label><input id="setDisputeCourt" class="input" value="${esc(profile.defaultDisputeCourt)}"></div><button class="btn secondary block" type="submit">保存合同抬头</button></form></div><div class="card"><div class="card-title">数据备份</div><div class="notice warn">建议每周导出一次完整备份，并保存到 iCloud、微信文件或其他可靠位置。恢复会覆盖当前数据。</div><button id="backupAll" class="btn block">导出完整 JSON 备份</button><label class="btn secondary block" style="display:block;text-align:center;margin-top:8px" for="restoreFile">从 JSON 备份恢复</label><input id="restoreFile" class="hidden" type="file" accept=".json,application/json"></div><div class="card"><div class="card-title">当前数据量</div><div class="grid-3"><div class="metric compact"><div class="label">商品</div><div class="value">${counts.products}</div></div><div class="metric compact"><div class="label">销售单</div><div class="value">${counts.sales}</div></div><div class="metric compact"><div class="label">调借单</div><div class="value">${counts.loans}</div></div></div></div><div class="card"><div class="card-title danger-text">危险操作</div><button id="clearAll" class="btn danger block">清空全部业务数据</button></div><div class="notice">版本：1.4 Cloud<br>本机使用 IndexedDB 加速，业务数据自动同步到 Cloudflare R2。</div><button id="syncNow" class="btn secondary block" style="margin-top:8px">立即同步云端</button><button id="logoutCloud" class="btn secondary block" style="margin-top:8px">退出云端账号</button>`;
  ...
  $('#backupAll').onclick=backupAll;$('#restoreFile').onchange=restoreAll;$('#clearAll').onclick=clearAllData;if($('#syncNow'))$('#syncNow').onclick=async()=>{await CloudSync.push();showToast('云端同步完成');};if($('#logoutCloud'))$('#logoutCloud').onclick=()=>CloudSync.logout();
}
async function backupAll(){
  const data={app:'漠翠进销存',version:'1.4',exportedAt:nowISO(),stores:{}};for(const s of STORES)data.stores[s]=await dbAll(s);downloadBlob(JSON.stringify(data,null,2),`漠翠进销存完整备份_${new Date().toISOString().slice(0,10)}.json`,'application/json');showToast('备份文件已导出');
}
...
window.addEventListener('DOMContentLoaded',()=>init().catch(err=>{console.error(err);document.body.innerHTML=`<div style="padding:20px">系统启动失败：${esc(err.message)}</div>`;}));
